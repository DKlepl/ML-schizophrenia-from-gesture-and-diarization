...
```{r}
info = get_info(gesture_files[1], type="g")
actigraph = read_actigraph(path=gesture_files[1])
...
#plot where eyeballing fails
plot(actigraph$PsychologistJerkLeft, type="l", col="navy")
x_left = as.data.frame(findpeaks(actigraph$PsychologistJerkLeft, minpeakheight = 2, npeaks = 100, minpeakdistance = 20))
points(x_left[, 2], x_left[, 1],col="red", pch=20)

ggplot(actigraph[1:nrow(actigraph),])+
   geom_line(aes((1:nrow(actigraph) )/100,PsychologistJerkLeft),color="blue")
```
...
dia = read.csv("raw_data/Diarization/103.csv")
```

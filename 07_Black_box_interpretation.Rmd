...
resp1_nn = variable_response(expl_nn, variable = "iqr_diff")
resp1_bayes = variable_response(expl_bayes, variable = "iqr_diff")
resp1_log = variable_response(expl_log, variable = "iqr_diff")
resp1_rf = variable_response(expl_rf, variable = "iqr_diff")
resp1_xgb = variable_response(expl_xgb, variable = "iqr_diff")

plot_iqr = plot(resp1_nn, resp1_bayes, resp1_log, resp1_rf, resp1_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of iqr_diff")
plot_iqr
...
resp2_nn = variable_response(expl_nn, variable = "sum_diff")
resp2_bayes = variable_response(expl_bayes, variable = "sum_diff")
resp2_log = variable_response(expl_log, variable = "sum_diff")
resp2_rf = variable_response(expl_rf, variable = "sum_diff")
resp2_xgb = variable_response(expl_xgb, variable = "sum_diff")


plot_sum = plot(resp2_nn, resp2_bayes, resp2_log, resp2_rf, resp2_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of sum_diff")
plot_sum
...
```{r}
#important for avnnet, 
resp3_nn = variable_response(expl_nn, variable = "median_diff")
resp3_bayes = variable_response(expl_bayes, variable = "median_diff")
resp3_log = variable_response(expl_log, variable = "median_diff")
resp3_rf = variable_response(expl_rf, variable = "median_diff")
resp3_xgb = variable_response(expl_xgb, variable = "median_diff")


plot_median = plot(resp3_nn, resp3_bayes, resp3_log, resp3_rf, resp3_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of median_diff")
plot_median
...
```{r}
#important for avnnet, 
resp4_nn = variable_response(expl_nn, variable = "maxL")
resp4_bayes = variable_response(expl_bayes, variable = "maxL")
resp4_log = variable_response(expl_log, variable = "maxL")
resp4_rf = variable_response(expl_rf, variable = "maxL")
resp4_xgb = variable_response(expl_xgb, variable = "maxL")


plot_maxL = plot(resp4_nn, resp4_bayes, resp4_log, resp4_rf, resp4_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of maxL")
plot_maxL
...
```{r}
#important for avnnet, 
resp5_nn = variable_response(expl_nn, variable = "mad_diff")
resp5_bayes = variable_response(expl_bayes, variable = "mad_diff")
resp5_log = variable_response(expl_log, variable = "mad_diff")
resp5_rf = variable_response(expl_rf, variable = "mad_diff")
resp5_xgb = variable_response(expl_xgb, variable = "mad_diff")


plot_mad = plot(resp5_nn, resp5_bayes, resp5_log, resp5_rf, resp5_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of mad_diff")
plot_mad
...
```{r}
#important for avnnet, 
resp6_nn = variable_response(expl_nn, variable = "LAM_diff")
resp6_bayes = variable_response(expl_bayes, variable = "LAM_diff")
resp6_log = variable_response(expl_log, variable = "LAM_diff")
resp6_rf = variable_response(expl_rf, variable = "LAM_diff")
resp6_xgb = variable_response(expl_xgb, variable = "LAM_diff")


plot_LAM = plot(resp6_nn, resp6_bayes, resp6_log, resp6_rf, resp6_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of LAM_diff")
plot_LAM
...
```{r}
#important for avnnet, 
resp7_nn = variable_response(expl_nn, variable = "var_diff")
resp7_bayes = variable_response(expl_bayes, variable = "var_diff")
resp7_log = variable_response(expl_log, variable = "var_diff")
resp7_rf = variable_response(expl_rf, variable = "var_diff")
resp7_xgb = variable_response(expl_xgb, variable = "var_diff")


plot_var = plot(resp7_nn, resp7_bayes, resp7_log, resp7_rf, resp7_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of var_diff")
plot_var
...
max_diff
```{r}
resp8_nn = variable_response(expl_nn, variable = "max_diff")
resp8_bayes = variable_response(expl_bayes, variable = "max_diff")
resp8_log = variable_response(expl_log, variable = "max_diff")
resp8_rf = variable_response(expl_rf, variable = "max_diff")
resp8_xgb = variable_response(expl_xgb, variable = "max_diff")


plot_max = plot(resp8_nn, resp8_bayes, resp8_log, resp8_rf, resp8_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of max_diff")
plot_max
...
range_diff
```{r}
resp9_nn = variable_response(expl_nn, variable = "range_diff")
resp9_bayes = variable_response(expl_bayes, variable = "range_diff")
resp9_log = variable_response(expl_log, variable = "range_diff")
resp9_rf = variable_response(expl_rf, variable = "range_diff")
resp9_xgb = variable_response(expl_xgb, variable = "range_diff")


plot_range = plot(resp9_nn, resp9_bayes, resp9_log, resp9_rf, resp9_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of range_diff")
plot_range
...
sd_diff
```{r}
resp10_nn = variable_response(expl_nn, variable = "sd_diff")
resp10_bayes = variable_response(expl_bayes, variable = "sd_diff")
resp10_log = variable_response(expl_log, variable = "sd_diff")
resp10_rf = variable_response(expl_rf, variable = "sd_diff")
resp10_xgb = variable_response(expl_xgb, variable = "sd_diff")


plot_sd = plot(resp10_nn, resp10_bayes, resp10_log, resp10_rf, resp10_xgb)+
  theme_few()+
  guides(shape=F)+
  labs(title = "PDP of sd_diff")
plot_sd
